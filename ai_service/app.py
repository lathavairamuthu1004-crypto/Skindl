...
try:
    ...
    print("AI ENGINE: Using optimized Computer Vision (OpenCV) for diagnostic inference.")
...
def analyze_image_cv(img_array):
    ...
    cv2.imwrite("debug_input.jpg", img_bgr) # Log the processed image
...
def predict():
    try:
        ...
        analysis = analyze_image_cv(center)
        top_result = analysis[0]
        
        progression = get_progression(top_result['label'])

        return jsonify({
            "condition": top_result['label'],
            "confidence": f"{top_result['prob']}%",
            "all_scores": analysis[:5], # Return top 5
            "progression": progression,
            "method": "TensorFlow-Optimized CV Engine"
        })
